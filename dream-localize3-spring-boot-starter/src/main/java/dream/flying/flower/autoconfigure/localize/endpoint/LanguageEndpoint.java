 ...
 *
 * <p>
 * The enabled filter is forwarded to the front-end via
 * {@code LanguageQuery.enabled} query parameter. Caller can pass
 * {@code enabled=1} for enabled-only dropdown, or omit for all records. Default
 * {@code list(query)} behavior from the inherited AbstractController is
 * preserved.
 * </p>
 *
 * @author 飞花梦影
...
public class LanguageEndpoint extends AbstractController<LanguageEntity, LanguageVO, LanguageQuery, LanguageService> {
	...
	@Operation(summary = "查询", description = "分页或不分页查询语言列表,启用过滤通过LanguageQuery.enabled参数由前端传递,不传默认返回所有", method = "GET")
	@Override
