package dream.flying.flower.autoconfigure.i18n.service.impl;
...
public class LocalizationServiceImpl extends AbstractServiceImpl<LocalizationEntity, LocalizationVO, LocalizationQuery,
	...
	public String getMessage(String langCode, String messageCode) {
		...
		// Query from database
		List<LocalizationEntity> messages =
				list(new LambdaQueryWrapper<LocalizationEntity>().eq(LocalizationEntity::getLang, langCode)
		...
		if (!messages.isEmpty()) {
			...
			try {
				redisTemplate.opsForValue()
						.set(cacheKey, messageContent, localizationProperties.getCacheExpireHours(), TimeUnit.HOURS);
			} catch (Exception e) {
	...
	public Map<String, String> getAllMessages(String langCode) {
		...
		try {
			if (!messageMap.isEmpty()) {
				...
				redisTemplate.expire(cacheKey, localizationProperties.getCacheExpireHours(), TimeUnit.HOURS);
			}
		} catch (Exception e) {
			// Redis connection failed, ignore cache operation
			e.printStackTrace();
